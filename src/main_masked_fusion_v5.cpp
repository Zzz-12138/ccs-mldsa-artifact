...
struct AStarEnumerationResult {
    ...
    std::array<int, kPolyN> ld_sorted_to_original{};
    std::array<float, kPolyN> leading_degree{};
};
...
public:
    ...
    explicit LdAStarPriorityEnumerator(
    ...
    AStarEnumerationResult Run(int64_t budget_limit) {
        ...
        for (int i = 0; i < kPolyN; ++i) {
            result.ld_sorted_to_original[i] = dims_[i].original_index;
            result.leading_degree[i] = dims_[i].leading_degree;
        }
        ...
        while (!frontier_.empty() && result.oracle_checks < budget_limit) {
            ...
            if (frontier_.size() > result.peak_frontier_size) {
                result.peak_frontier_size = frontier_.size();
            }
        }
    ...
    void Preprocess(
        ...
        for (int i = 0; i < kPolyN; ++i) {
            ...
            if (unique_scores.size() > 1) {
                dim.leading_degree = unique_scores[0] - unique_scores[1];
            } else {
                dim.leading_degree = std::numeric_limits<float>::infinity();
            }
...
bool load_binary(const std::string& path, std::vector<T>& data) {
    ...
    std::error_code ec;
    fs::path p(path);
    std::string abs_path = fs::absolute(p, ec).string();
    uintmax_t fsize = fs::file_size(p, ec);

    std::cout << "[FILE LOADING] Name: " << p.filename().string() << "\n"
              << "               Path: " << abs_path << "\n"
              << "               Size: " << std::fixed << std::setprecision(2)
              << (fsize / (1024.0 * 1024.0)) << " MB (" << fsize << " bytes)\n";

    data.resize(static_cast<size_t>(sz) / sizeof(T));
    bool success = f.read(reinterpret_cast<char*>(data.data()), sz).good();

    if (success) {
        std::cout << "[FILE LOADED]  Name: " << p.filename().string() << "\n";
    }
...
std::string find_file_with_pattern(const std::string& directory, const std::string& prefix, const std::string& suffix) {
    ...
    for (const auto& entry : fs::directory_iterator(directory)) {
        ...
        const bool prefix_ok = filename.compare(0, prefix.size(), prefix) == 0;
        const bool suffix_ok = filename.compare(filename.size() - suffix.size(), suffix.size(), suffix) == 0;

        if (prefix_ok && suffix_ok) matches.push_back(entry.path().string());
    }
...
int extract_trace_count(const std::string& filepath) {
    ...
    auto parse_number_before = [&](size_t pos) -> int {
        size_t start = pos;
    ...
    size_t k_pos = filename.find("k_");
    if (k_pos != std::string::npos) {
        int n = parse_number_before(k_pos);
        if (n > 0) return n * 1000;
    }

    size_t tr_pos = filename.find("tr_");
    if (tr_pos != std::string::npos) {
        int n = parse_number_before(tr_pos);
        if (n > 0) return n;
    }

    size_t e_pos = filename.find("e");
    while (e_pos != std::string::npos) {
        if (e_pos > 0 && e_pos + 1 < filename.size() &&
            std::isdigit(static_cast<unsigned char>(filename[e_pos - 1])) &&
            std::isdigit(static_cast<unsigned char>(filename[e_pos + 1]))) {
            int base = parse_number_before(e_pos);
            ...
            int exp = std::stoi(filename.substr(exp_start, exp_end - exp_start));
            int mult = 1;
            for (int i = 0; i < exp; ++i) mult *= 10;
            return base * mult;
        }
        e_pos = filename.find("e", e_pos + 1);
    }

    throw std::runtime_error("Could not parse trace count from filename: " + filename);
}

inline float fisher_z(float rho) {
    return std::atanhf(std::clamp(rho, -0.9999f, 0.9999f));
}

struct FusionEval {
    float ld = -std::numeric_limits<float>::infinity();
    int32_t best_g = 0;
    std::array<float, 3> alpha{0.0f, 0.0f, 0.0f};
    std::vector<int32_t> topK;
    std::vector<float> topScores;
};

bool preserves_top_prefix(const FusionEval& base, const FusionEval& candidate, int prefix) {
    if (base.topK.empty() || candidate.topK.empty()) return false;
    prefix = std::min(prefix, static_cast<int>(std::min(base.topK.size(), candidate.topK.size())));

    for (int i = 0; i < prefix; ++i) {
        bool found = false;
        for (int j = 0; j < prefix; ++j) {
            if (base.topK[i] == candidate.topK[j]) {
                found = true;
                break;
            }
        }
...
FusionEval search_ld_fusion(
    ...
    const std::vector<float>* z_intt,
    int max_k,
    int grid = 20,
    int ld_m = 4) {
    FusionEval best;
    const bool use_intt = (z_intt != nullptr);
    ...
    #pragma omp parallel
    {
        ...
        for (int a = 0; a <= grid; ++a) {
            for (int b = 0; b <= grid - a; ++b) {
                const int c = use_intt ? (grid - a - b) : 0;
                if (!use_intt && a + b != grid) continue;
                ...
                for (int g = 0; g < Consts::Q; ++g) {
                    float score = static_cast<float>(a) * z_ay[g] + static_cast<float>(b) * z_cs1[g];
                    if (use_intt) score += static_cast<float>(c) * (*z_intt)[g];
                    score /= static_cast<float>(grid);
                ...
                if (ld > local.ld) {
                    ...
                    local.alpha = {
                        a / static_cast<float>(grid),
                        b / static_cast<float>(grid),
                        c / static_cast<float>(grid)
                    };
...
int count_matches_ntt(const std::vector<int32_t>& truth, const std::array<int32_t, 256>& candidate) {
    ...
    for (int i = 0; i < Consts::N_ROUNDS; ++i) {
        int32_t true_s1 = truth[i];
        if (true_s1 < 0) true_s1 += Consts::Q;
    ...
    try {
        if (argc < 11) {
            throw std::runtime_error(
                "Usage: ./main_masked_fusion_v5 <data_dir> <instances> <mode:44|65|87> "
                "<opt:o0|o3> <sampling:25|100> <threads> <start> <end> <step> "
                "<max_k> [use_intt:0|1] [results_csv]");
        }
        ...
        cfg.request_intt = (argc >= 12 && std::stoi(argv[11]) != 0);
        ...
        if (argc >= 13) {
            cfg.results_path = argv[12];
        } else {
            cfg.results_path = "results/mldsa_" + std::to_string(cfg.mode) +
                               "_masked_fusion_v5_" + cfg.opt_level + "_" +
                               cfg.sampling_tag + ".csv";
        }

        ensure_parent_dir(cfg.results_path);
        cfg.results_path = unique_filename(cfg.results_path);
        std::cout << "[RESULT FILE] " << cfg.results_path << "\n";
        ...
        if (!load_binary(data_path(cfg.data_dir, "poly0_s1_ntt_true.bin").string(), s1_true)) {
            throw std::runtime_error("Failed to load poly0_s1_ntt_true.bin");
        }
        ...
        const std::string cs1_s0_file = find_file_with_pattern(
            cfg.data_dir,
            "poly0_share0_cs1_" + cfg.sampling_tag + "_",
            suffix);
        const std::string cs1_s1_file = find_file_with_pattern(
            cfg.data_dir,
            "poly0_share1_cs1_" + cfg.sampling_tag + "_",
            suffix);

        std::vector<float> cs1_share0, cs1_share1;
        if (cs1_s0_file.empty() || !load_binary(cs1_s0_file, cs1_share0)) {
            throw std::runtime_error("Failed to load CS1 share0");
        }
        if (cs1_s1_file.empty() || !load_binary(cs1_s1_file, cs1_share1)) {
            throw std::runtime_error("Failed to load CS1 share1");
        }

        const int cs1_traces = extract_trace_count(cs1_s0_file);
        const int T_cs1 = static_cast<int>(cs1_share0.size() / (static_cast<size_t>(cs1_traces) * Consts::N_ROUNDS));
        ...
        const std::string ay_s0_file = find_file_with_pattern(
            cfg.data_dir,
            "ay_poly0_share0_" + cfg.sampling_tag + "_",
            suffix);
        const std::string ay_s1_file = find_file_with_pattern(
            cfg.data_dir,
            "ay_poly0_share1_" + cfg.sampling_tag + "_",
            suffix);

        std::vector<float> ay_share0, ay_share1;
        if (ay_s0_file.empty() || !load_binary(ay_s0_file, ay_share0)) {
            throw std::runtime_error("Failed to load AY share0");
        }
        if (ay_s1_file.empty() || !load_binary(ay_s1_file, ay_share1)) {
            throw std::runtime_error("Failed to load AY share1");
        }

        const int ay_traces = extract_trace_count(ay_s0_file);
        const int T_ay = static_cast<int>(ay_share0.size() / (static_cast<size_t>(ay_traces) * K_ROWS * Consts::N_ROUNDS));

        bool has_intt = false;
        int intt_traces = 0;
        int T_intt = 0;
        std::vector<float> intt_share0, intt_share1;

        if (cfg.request_intt) {
            const std::string intt_s0_file = find_file_with_pattern(
                cfg.data_dir,
                "poly0_share0_intt_cs1_" + cfg.sampling_tag + "_",
                suffix);
            const std::string intt_s1_file = find_file_with_pattern(
                cfg.data_dir,
                "poly0_share1_intt_cs1_" + cfg.sampling_tag + "_",
                suffix);

            if (!intt_s0_file.empty() && !intt_s1_file.empty()) {
                if (!load_binary(intt_s0_file, intt_share0)) throw std::runtime_error("Failed to load INTT share0");
                if (!load_binary(intt_s1_file, intt_share1)) throw std::runtime_error("Failed to load INTT share1");

                intt_traces = extract_trace_count(intt_s0_file);
                T_intt = static_cast<int>(intt_share0.size() / (static_cast<size_t>(intt_traces) * Consts::N_ROUNDS));
                has_intt = true;
            }
        }

        const int physical_traces = has_intt ? std::min({cs1_traces, ay_traces, intt_traces})
                                             : std::min(cs1_traces, ay_traces);

        std::cout << "[TRACE COUNT] CS1=" << cs1_traces
          << " AY=" << ay_traces;

        if (has_intt) {
            std::cout << " INTT=" << intt_traces;
        }

        std::cout << " Requested=" << (cfg.instances * cfg.end)
        ...
        for (int req = cfg.start; req <= cfg.end; req += cfg.step) {
            trace_steps.push_back(req);
        }
        ...
        res_file << "instance,num_traces,Succ_cs1,Succ_ay,Succ_intt,Succ_LD,Succ_LD_1e6,"
                 << "InttAvailable,InttUsedCoeffs,time_spent\n";
        res_file.flush();
        ...
        std::cout << "Mode=ML-DSA-" << cfg.mode
                  ...
                  << " K_ROWS=" << K_ROWS
                  << " INTT=" << (has_intt ? "available" : "disabled")
        ...
        for (int inst = 0; inst < cfg.instances; ++inst) {
            ...
            std::vector<float> inst_cs1_s0(static_cast<size_t>(max_N) * Consts::N_ROUNDS * T_cs1);
            std::vector<float> inst_cs1_s1(static_cast<size_t>(max_N) * Consts::N_ROUNDS * T_cs1);
            std::vector<int32_t> inst_cs1_labels(static_cast<size_t>(max_N) * Consts::N_ROUNDS);

            std::vector<float> inst_ay_s0(static_cast<size_t>(max_N) * K_ROWS * Consts::N_ROUNDS * T_ay);
            std::vector<float> inst_ay_s1(static_cast<size_t>(max_N) * K_ROWS * Consts::N_ROUNDS * T_ay);
            ...
            std::vector<float> inst_intt_s0;
            std::vector<float> inst_intt_s1;
            if (has_intt) {
                inst_intt_s0.resize(static_cast<size_t>(max_N) * Consts::N_ROUNDS * T_intt);
                inst_intt_s1.resize(static_cast<size_t>(max_N) * Consts::N_ROUNDS * T_intt);
            }
            ...
            for (int i = 0; i < max_N; ++i) {
                ...
                for (int r = 0; r < Consts::N_ROUNDS; ++r) {
                    inst_cs1_labels[i * Consts::N_ROUNDS + r] =
                        cs1_c_labels[original_idx * Consts::N_ROUNDS + r];

                    for (int t = 0; t < T_cs1; ++t) {
                        inst_cs1_s0[static_cast<size_t>(i) * Consts::N_ROUNDS * T_cs1 + r * T_cs1 + t] =
                            cs1_share0[static_cast<size_t>(original_idx) * Consts::N_ROUNDS * T_cs1 + r * T_cs1 + t];
                        inst_cs1_s1[static_cast<size_t>(i) * Consts::N_ROUNDS * T_cs1 + r * T_cs1 + t] =
                            cs1_share1[static_cast<size_t>(original_idx) * Consts::N_ROUNDS * T_cs1 + r * T_cs1 + t];
                    }

                    inst_ay_c_ntt[i * Consts::N_ROUNDS + r] =
                        ay_c_ntt[original_idx * Consts::N_ROUNDS + r];
                    inst_ay_z_ntt[i * Consts::N_ROUNDS + r] =
                        ay_z_ntt[original_idx * Consts::N_ROUNDS + r];
                    ...
                    for (int k = 0; k < K_ROWS; ++k) {
                        for (int t = 0; t < T_ay; ++t) {
                            ...
                            const size_t dst_idx =
                                ...
                                (k * Consts::N_ROUNDS + r) * T_ay + t;
                            inst_ay_s0[dst_idx] = ay_share0[src_idx];
                            inst_ay_s1[dst_idx] = ay_share1[src_idx];
                        }
                    }

                    if (has_intt) {
                        for (int t = 0; t < T_intt; ++t) {
                            inst_intt_s0[static_cast<size_t>(i) * Consts::N_ROUNDS * T_intt + r * T_intt + t] =
                                intt_share0[static_cast<size_t>(original_idx) * Consts::N_ROUNDS * T_intt + r * T_intt + t];
                            inst_intt_s1[static_cast<size_t>(i) * Consts::N_ROUNDS * T_intt + r * T_intt + t] =
                                intt_share1[static_cast<size_t>(original_idx) * Consts::N_ROUNDS * T_intt + r * T_intt + t];
                        }
                    }
                }
            }
            ...
            for (int req_traces : trace_steps) {
                ...
                for (int r = 0; r < Consts::N_ROUNDS; ++r) {
                    ...
                    int32_t true_s1 = s1_true[r];
                    if (true_s1 < 0) true_s1 += Consts::Q;

                    std::vector<float> sc_cs1(Consts::Q, -1.0f);
                    std::vector<float> sc_ay(Consts::Q, -1.0f);
                    std::vector<float> sc_intt(Consts::Q, -1.0f);
                    std::vector<int> best_t_intt(Consts::Q, 0);
                    std::vector<float> d_m1(Consts::Q), d_m2(Consts::Q), d_m3(Consts::Q), d_m4(Consts::Q);

                    gpu_solve_round_cublas_fusion(
                        is_new_instance,
                        inst_cs1_s0,
                        inst_cs1_s1,
                        inst_cs1_labels,
                    ...
                    if (has_intt) {
                        const int32_t b_intt = static_cast<int32_t>(
                            std::distance(sc_intt.begin(), std::max_element(sc_intt.begin(), sc_intt.end())));
                        if (b_intt == true_s1 || b_intt == Consts::Q - true_s1) ++s_intt;
                    }

                    std::vector<float> z_ay(Consts::Q);
                    std::vector<float> z_cs1(Consts::Q);
                    std::vector<float> z_intt(Consts::Q);

                    #pragma omp parallel for
                    for (int g = 0; g < Consts::Q; ++g) {
                        z_ay[g] = fisher_z(sc_ay[g]);
                        z_cs1[g] = fisher_z(sc_cs1[g]);
                        if (has_intt) z_intt[g] = fisher_z(sc_intt[g]);
                    }

                    FusionEval base = search_ld_fusion(z_ay, z_cs1, nullptr, cfg.max_k);
                    FusionEval chosen = base;

                    if (has_intt) {
                        FusionEval tri = search_ld_fusion(z_ay, z_cs1, &z_intt, cfg.max_k);
                        const float required_gain = std::max(0.25f, 0.05f * std::max(1.0f, std::abs(base.ld)));
                        const bool useful_intt =
                            tri.alpha[2] > 0.0f &&
                            tri.best_g == base.best_g &&
                            preserves_top_prefix(base, tri, std::min(3, cfg.max_k)) &&
                            tri.ld >= base.ld + required_gain;
                    ...
                    if (chosen.best_g == true_s1) ++s_ld;

                    topK_ld[r] = chosen.topK;
                    score_ld[r] = chosen.topScores;
                }

                const auto sieve_res = mldsa_sieve::EnumerateLdAStarKey(score_ld, topK_ld, 1000000LL, eta_bound);
                int s_ld_1e6 = s_ld;
                if (sieve_res.found) {
                    s_ld_1e6 = count_matches_ntt(s1_true, sieve_res.found_ntt);
                }

                const auto step_end = std::chrono::high_resolution_clock::now();
                const double time_spent = std::chrono::duration<double>(step_end - step_start).count();
                ...
                std::cout << "inst=" << (inst + 1)
                          ...
                          << " ld=" << s_ld
                          << " sieve=" << s_ld_1e6
                          << " intt_used=" << intt_used_coeffs
                          << "\n";
            }
        }

        const auto global_end = std::chrono::high_resolution_clock::now();
        std::cout << "Done in " << std::chrono::duration<double>(global_end - global_start).count() << "s\n";
